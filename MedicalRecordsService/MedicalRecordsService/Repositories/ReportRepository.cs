...
namespace MedicalRecordsService.Repositories
{
    public class ReportRepository : IReportRepository
    {
        ...
        public void UpdateReportByPatientId(int patientId, Report updatedReport)
        {
            var report = (
                from r in _context.Reports
                join a in _context.Appointments
            ...
            if (report == null)
                throw new KeyNotFoundException("Report not found for this patient");

            // ✅ Update only editable fields
            report.Symptoms = updatedReport.Symptoms ?? report.Symptoms;
            report.Diagnosis = updatedReport.Diagnosis ?? report.Diagnosis;
            report.Prescription = updatedReport.Prescription ?? report.Prescription;
            report.Remarks = updatedReport.Remarks ?? report.Remarks;

            // ✅ Optional Date handling
            if (updatedReport.ReportDate != default)
